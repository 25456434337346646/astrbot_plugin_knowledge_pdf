...
async def fetch_content(context: Context, key: str) -> str | None:
    """Retrieve knowledge base content for *key* via Tool Proxy."""
    try:
        # Try built-in invoke_tool / tool_manager
        inst = getattr(context, "inst", None)
        manager = getattr(context, "tool_manager", None) or getattr(inst, "tool_manager", None)
        if manager:
...
@register("knowledge_pdf", "Master", "极简 PDF 导出器", "1.1.0")
class KnowledgePDFPlugin(Star):
    ...
    async def pdf_smart(self, event: AstrMessageEvent) -> MessageEventResult:
        """智能化 PDF 指令：有参数则搜索知识库，无参数则转换上一条回复。"""
        ...
        if len(args) > 1:
            ...
            mgr = getattr(self.context, "message_manager", None) or getattr(self.context.inst, "message_manager", None)
            ...
            if not content:
                return event.plain_result("未能捕捉到上一条对话内容，请先让我为您检索一次。")
