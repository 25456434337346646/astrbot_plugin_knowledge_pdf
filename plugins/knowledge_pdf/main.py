...
class KnowledgePDFPlugin(Star):
    ...
    def render_pdf(self, content: str, title_text: str, output_path: Path):
        ...
        for line in lines:
            ...
            # [CRITICAL FIX] Filter out extremely long base64/image tags that break ReportLab
            if "base64" in line and len(line) > 500:
                line = "[图片数据已过滤，仅支持文本导出]"
            ...
            try:
                ...
                # Fallback: if Paragraph still fails, try to strip everything and just put plain text
                story.append(Paragraph("内容片段解析异常，已跳过损坏字符。", styles['body']))
        
        # Footer
        story.append(Spacer(1, 48))
        story.append(Paragraph("--- Generated by AstrBot (Safe Render Mode) ---", styles['footer']))
    ...
    async def pdf_handle(self, event: AstrMessageEvent) -> MessageEventResult:
        ...
        try:
            ...
            # Some AstrBot versions do not support passing a chain via the constructor or `chain_result`.
            # To ensure compatibility, we instantiate an empty MessageEventResult and manually set its `chain`.
            result = MessageEventResult()
            result.chain = [File(str(tmp_path))]
            return result
        except Exception as e:
